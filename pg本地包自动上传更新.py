...
phone = "+86 15861603901"
channel = 'https://t.me/PandaGroovePG'
group_username = '@pgzdsc'  # 修改为实际的群组用户名
download_path = '/mnt/sata2-4/docker/vod/player/xzqzy/lib1/pgdown'  # 下载路径
target_path = '/mnt/sata2-4/docker/vod/player/xzqzy/lib1'  # 目标路径
filter = r".*\.zip"  # 文件过滤规则

# 确保 session 文件路径正确
session_file = '/www/pgscts/pg.session'
        ...
        if message.document and isinstance(message.document, Document):
            if message.document.mime_type == 'application/zip':
                ...
                if file_name and re.match(filter, file_name):
                    # 检查文件是否为当天的文件
                    message_date = message.date.astimezone().date()
